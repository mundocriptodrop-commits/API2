                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        setCompanyFormData({ ...companyFormData, createUser: !companyFormData.createUser });
                      }}
                      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                        companyFormData.createUser ? 'bg-blue-500' : 'bg-gray-200'
                      }`}
                      role="switch"
                      aria-checked={companyFormData.createUser}
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                          companyFormData.createUser ? 'translate-x-5' : 'translate-x-0'
                        }`}
                      />
                    </button>
                  </div>
