...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body: SendCarouselRequest = await req.json();
    ...
    if (!body.carousel || !Array.isArray(body.carousel) || body.carousel.length === 0) {
      return new Response(
        JSON.stringify({ error: "Campo 'carousel' é obrigatório e deve ser um array não vazio" }),
        {
          status: 400,
          headers: {
            ...corsHeaders,
    ...
    const response = await fetch("https://sender.uazapi.com/send/carousel", {
      method: "POST",
      headers: {
    ...
    return new Response(
      JSON.stringify({ 
        error: error.message || "Erro ao processar requisição de carrossel",
